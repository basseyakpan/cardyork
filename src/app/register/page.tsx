  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!firstNameOk || !lastNameOk) {
      dispatch(
        showToast({
          message:
            "Please enter your full name (at least 3 characters each for first and last name).",
          type: "error",
        }),
      );
      return;
    ...
    try {
      ...
      dispatch(
        showToast({
          message:
            "OTP Sent! 📧 Check your email for the 6-digit activation code.",
          type: "success",
        }),
      );
    ...
    <form onSubmit={handleSubmit} className="space-y-4 animate-fade-in">
      ...
      <div className="input-group">
        ...
        {fullName.trim() && (!firstNameOk || !lastNameOk) && (
          <p className="text-xs text-error flex items-center gap-1 mt-1 font-medium">
            <FiAlertCircle className="w-3 h-3" /> Min 3 chars each for first and
            last name
          </p>
        )}
      </div>
        ...
        {password && (
          <div className="mt-3 space-y-1.5 p-3 rounded-xl bg-surface-container/50 border border-primary/5">
            {passwordRules.map((rule, i) => (
              ...
              >
                <div
                  className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${rule.valid ? "bg-secondary text-white" : "bg-surface-container-high text-on-surface-variant"}`}
                >
                  {rule.valid && <FiCheck className="w-2.5 h-2.5" />}
                </div>
                <span
                  className={
                    rule.valid ? "text-secondary" : "text-on-surface-variant"
                  }
                >
                  {rule.label}
                </span>
      ...
      <div className="flex items-start gap-3 pt-2">
        <input
          ...
          className="mt-1 w-4 h-4 text-primary bg-surface-container border-primary/20 rounded focus:ring-primary focus:ring-2"
        />
        <label
          htmlFor="terms"
          className="text-sm font-medium leading-relaxed text-on-surface-variant cursor-pointer"
        >
...
// ─────────────────────────────────────────────
// Resume Pending Registration Dialog
// ─────────────────────────────────────────────
  ...
  return (
    <>
      <p className="text-center text-xs text-on-surface-variant mt-2 font-medium">
        ...
        >
          <FiRefreshCw className="w-3 h-3" /> Resume verification
        </button>
      </p>
...
function OtpStep({ tempEmail, onActivate, onResend }: any) {
  ...
  return (
    <form
      ...
      className="space-y-6 animate-fade-in text-center"
    >
      ...
      <div className="input-group text-left mt-6">
        ...
        <input
          ...
          className="input-field text-center text-2xl tracking-[0.4em] h-14 font-mono font-bold bg-surface-container"
          maxLength={6}
          required
          autoFocus
        />
...
function PinChoiceStep({
  ...
  const handleSetupNow = async () => {
    try {
      ...
      dispatch(
        showToast({
          message: "OTP Sent! A verification code has been sent for PIN setup.",
          type: "success",
        }),
      );
      onSendPinOtp();
    ...
    <div className="text-center space-y-6 animate-fade-in">
      ...
      <div>
        ...
        <p className="text-on-surface-variant mt-2 text-sm leading-relaxed">
          Set up a 4-digit transaction PIN to securely authorize your trades and
          withdrawals.
        </p>
      </div>
      <div className="space-y-3 pt-4">
        ...
        <button
          onClick={onSkip || (() => navigate("/login"))}
          className="btn btn-outline-primary w-full h-12 border-transparent hover:border-primary/20"
        >
          Skip for Later
        </button>
      </div>
...
function PinSetupStep({ tempUserId }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await dispatch(
        setPin({ id: tempUserId!, pin, token: pinToken }),
      ).unwrap();
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-on-surface-variant mt-2 text-sm leading-relaxed">
          Enter the authorization code sent to your email and set a new 4-digit
          PIN.
        </p>
      </div>
      <div className="space-y-4 text-left">
        <div className="input-group">
          ...
          <input
            ...
            className="input-field text-center tracking-[0.3em] font-mono font-bold"
            required
          />
        </div>
        <div className="input-group">
          ...
          <input
            ...
            maxLength={4}
            placeholder="••••"
            className="input-field text-center text-2xl h-14 tracking-widest font-bold"
            required
          />
        </div>
      </div>
...
export default function RegisterPage() {
  const dispatch = useAppDispatch();
  const { registrationStep, tempEmail, tempUserId } = useAppSelector(
    (s) => s.auth,
  );
  ...
  // When resuming from a persisted pending state, ensure localData.email is set
  useEffect(() => {
    if (tempEmail && !localData.email) {
      setLocalData((d) => ({ ...d, email: tempEmail }));
    }
  }, [tempEmail]);
    ...
    const handleAutoLogin = async () => {
      if (registrationStep === "completed") {
        if (localData.email && localData.password) {
          ...
          try {
            await dispatch(
              login({ email: localData.email, password: localData.password }),
            ).unwrap();
            setTimeout(() => {
            ...
            dispatch(
              showToast({
                message:
                  errMsg || "Your account is ready. Please log in manually.",
                type: "error",
              }),
            );
            setTimeout(() => {
          ...
          dispatch(
            showToast({
              message:
                "Registration Complete! ✅ Your account is fully set up. Please log in.",
              type: "success",
            }),
          );
          setTimeout(() => {
  ...
  const handleActivate = async (otp: string) => {
    try {
      const response: any = await dispatch(
        activateAccount({ email: tempEmail!, code: otp }),
      ).unwrap();
      const userid =
      ...
      if (!userid && !tempUserId) {
        throw new Error(
          "User ID not found in activation response. Please try logging in.",
        );
      }

      dispatch(
        showToast({
          message: "Email Verified! ✅ Setting up your profile…",
          type: "success",
        }),
      );
      await dispatch(
      ...
      dispatch(
        showToast({
          message: "Profile Ready! ✨ Your profile has been set up.",
          type: "success",
        }),
      );
    } catch (err: any) {
  ...
  const handleResend = async () => {
    try {
      ...
      dispatch(
        showToast({
          message: "Code resent! Check your email again.",
          type: "success",
        }),
      );
    } catch (err: any) {
      const errMsg =
        err?.message ||
        (typeof err === "string" ? err : "Failed to resend code");
      dispatch(showToast({ message: errMsg, type: "error" }));
    }
  };
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="w-full lg:w-1/2 flex flex-col p-6 md:p-12 overflow-y-auto relative z-10">
        ...
        {registrationStep === "signup" && (
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-sm text-on-surface-variant hover:text-primary mb-6 relative z-10 w-max"
          >
        ...
        <div className="flex items-center gap-2 mb-6 max-w-md w-full mx-auto relative z-10">
          {(["signup", "activation", "pin"] as const).map((step, i) => {
            ...
            const currentIdx = steps.indexOf(
              registrationStep === "profile" ? "activation" : registrationStep,
            );
            const isActive = i <= currentIdx;
            return (
              <div key={step} className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full transition-all duration-300 ${isActive ? "bg-primary scale-125" : "bg-surface-container-highest"}`}
                />
                {i < 2 && (
                  <div
                    className={`h-px flex-1 w-8 transition-colors duration-300 ${i < currentIdx ? "bg-primary" : "bg-surface-container-highest"}`}
                  />
                )}
              </div>
            );
      ...
      <div className="hidden lg:flex w-1/2 relative items-center justify-center bg-surface-container border-l border-primary/10 overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg text-center p-12">
          ...
          <p className="text-on-surface-variant text-lg mb-10 leading-relaxed">
            Join thousands of smart traders converting their gift cards into
            cash at the highest rates.
          </p>
          ...
          <div className="flex flex-col items-start gap-5 max-w-xs mx-auto">
            {[
              ...
              <div
                key={item}
                className="flex items-center gap-4 bg-surface-container-high/50 w-full p-3 rounded-xl border border-primary/5"
              >
