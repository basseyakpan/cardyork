...
export function TradeGiftCard() {
        ...
        dispatch(fetchAssetRates({ userId, assetId: asset._id }))
          ...
          .then((fetchedRates) => {
            if (fetchedRates && fetchedRates.length > 0) {
              ...
              dispatch(
                showToast({
                  message: "This card has no active rates at the moment.",
                  type: "error",
                }),
              );
            }
          ...
          .catch(() => {
            dispatch(
              showToast({
                message: "Failed to fetch rates for this card.",
                type: "error",
              }),
            );
          });
  ...
  const assetRates = useMemo(() => {
    ...
    return rates.filter(
      (r) =>
        r.asset?._id === selectedAsset._id ||
        (r.asset as any) === selectedAsset._id,
    );
  }, [rates, selectedAsset]);
  ...
  if (selectedRate && amount) {
    ...
    if (selectedRate.fixRange) {
      const validAmounts = selectedRate.fixRange
        .split(",")
      ...
      isAmountValid =
        numAmount >= selectedRate.from && numAmount <= selectedRate.to;
  ...
  const calculatedAmount = isAmountValid
    ? Number(amount) * selectedRate.rate
    : 0;
    ...
    if (currentStep === "details") {
      ...
      if (uploadedImages.length === 0 && !cardCode) {
        dispatch(
          showToast({
            message:
              "Please upload at least one card image or provide the card code",
            type: "error",
          }),
        );
        return;
      ...
      if (requiresReceipt && receiptImages.length === 0) {
        dispatch(
          showToast({ message: "Please upload your receipt", type: "error" }),
        );
        return;
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (selectedAsset?.name?.toLowerCase().includes("tremendous")) {
        ...
        const chatId = await chatService.createOrGetChatSession(
          userId,
          userName,
          user?.email || "",
        );
...
${cardCode ? `Card Code: ${cardCode}\n` : ""}${cardPin ? `Card PIN: ${cardPin}\n` : ""}${comments ? `Comments: ${comments}` : ""}
        ...
        dispatch(
          showToast({
            message: "Your Tremendous trade has been sent to the support chat!",
            type: "success",
          }),
        );
      ...
      dispatch(
        showToast({
          message:
            "Trade submitted successfully! You'll be notified once it's processed.",
          type: "success",
        }),
      );
          ...
          {images.map((img: string, index: number) => (
            <div key={index} className="relative group">
              <img
                src={img}
                alt={`Upload ${index + 1}`}
                className="w-full h-24 object-cover rounded-xl border border-primary/20"
              />
              <button
                type="button"
          ...
          {steps.map((step, index) => (
            <div key={step.id} className="flex items-center">
              <div className="flex flex-col items-center">
                ...
                <span
                  className={`text-xs mt-2 hidden md:block font-bold ${index <= currentStepIndex ? "text-primary" : "text-on-surface-variant"}`}
                >
                  {step.label}
                </span>
              </div>
              {index < steps.length - 1 && (
                <div
                  className={`w-16 md:w-32 h-0.5 mx-2 ${index < currentStepIndex ? "bg-primary" : "bg-surface-container-highest"}`}
                />
              )}
            </div>
          ))}
                  ...
                  {[...assets]
                    ...
                    .map((asset) => {
                      const hasRates = rates.some(
                        (r) =>
                          r.asset?._id === asset._id ||
                          (r.asset as any) === asset._id,
                      );
                      return (
                        <button
                          key={asset._id}
                          ...
                          onClick={() => {
                            ...
                            dispatch(
                              fetchAssetRates({ userId, assetId: asset._id }),
                            )
                              .unwrap()
                              .then((fetchedRates) => {
                                if (fetchedRates && fetchedRates.length > 0) {
                              ...
                              .catch(() => {
                                dispatch(
                                  showToast({
                                    message:
                                      "Failed to fetch rates for this card.",
                                    type: "error",
                                  }),
                                );
                              });
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 font-bold text-center text-xs text-on-surface tracking-wide uppercase">
                            <div
                              className={`w-2 h-2 rounded-full ${(asset as any).cardActive === false ? "bg-red-500" : "bg-green-500"}`}
                            />
                            {asset.name}
                          </div>
                        </button>
          ...
          {currentStep === "details" && (
            <div className="animate-fade-in flex flex-col gap-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="input-group">
                  <label className={`input-label ${!country && "opacity-50"}`}>
                    Card Type *
                  </label>
                  <select
                  ...
                  >
                    <option value="" disabled>
                      {country ? "Select card type" : "Select country first"}
                    </option>
                    {availableTypes.map((type) => {
                      const typeRate = assetRates.find(
                        (r) => r.type === type && r.country === country,
                      );
                      return (
              ...
              {selectedRate && selectedRate.rateActive === false && (
                <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 text-sm font-bold flex items-start gap-3">
                  ...
                  <p className="flex-1">
                    We are unable to redeem {selectedAsset?.name} ({cardType})
                    at the moment due to an issue beyond our control. Our team
                    is working to resolve this as soon as possible.
                  </p>
                </div>
              )}
              ...
              <UploadSection
                label={
                  requiresReceipt ? "Card Images (optional)" : "Card Images *"
                }
            ...
            <div className="animate-fade-in flex flex-col gap-6">
                ...
                <div className="grid grid-cols-2 gap-y-4 pt-2">
                  ...
                  {cardCode && (
                    <div className="col-span-2">
                      ...
                      <p className="font-mono text-sm bg-surface-container-highest inline-block px-2 py-1 rounded">
                        {cardCode}
                      </p>
                    </div>
                  )}
              ...
              <div className="p-4 rounded-xl bg-secondary/10 border border-secondary/20 flex items-start gap-3">
                ...
                <p className="text-xs text-on-surface font-medium leading-relaxed">
                  By submitting this trade, you confirm that the provided
                  details and images are accurate.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="flex justify-between mt-10 pt-6 border-t border-primary/10">
            {currentStep !== "select" ? (
              <button
                ...
                disabled={isUploading || tradeLoading}
              >
