...
export default function Footer() {
  const [iphoneDownloadText, setIphoneDownloadText] = useState(null);
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-x-6 gap-y-12 lg:gap-8">
            ...
            <div className="flex flex-col gap-6 col-span-2 md:col-span-4 lg:col-span-2">
              ...
              <p className="text-on-surface-variant text-sm leading-relaxed max-w-[320px]">
                Cardyork is a technology-driven platform focused on simplifying
                gift card transactions through secure systems, transparent
                processes, and exceptional customer experience. We are dedicated
                to building trusted digital solutions that users can rely on
                every day.
              </p>

              <div className="mt-4">
                <h4 className="text-on-surface text-base font-bold mb-4">
                  Trade on the Go
                </h4>
                <div className="flex flex-wrap gap-3">
                  <Link
                    ...
                    className={`inline-flex items-center gap-2 bg-black text-white px-4 py-2.5 rounded-lg hover:opacity-90 transition-opacity text-left cursor-pointer border-none ${iphoneDownloadText && "animate-pulse"}`}
                  >
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-2 bg-black text-white px-4 py-2.5 rounded-lg hover:opacity-90 transition-opacity no-underline"
                  >
              ...
              <ul className="flex flex-col gap-3 list-none">
                {FOOTER_LINKS.company.map((l) => (
                  <li key={l.href}>
                    <Link
                      href={l.href}
                      className="text-on-surface-variant text-sm hover:text-primary transition-colors no-underline"
                    >
                      {l.label}
                    </Link>
                  </li>
